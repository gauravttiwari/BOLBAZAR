...
function OrderTrackingContent() {
  ...
  useEffect(() => {
    const onVoiceCancelOrder = () => {
      setPendingCancel(true);
    };
    window.addEventListener("voice-cancel-order", onVoiceCancelOrder);
    return () => window.removeEventListener("voice-cancel-order", onVoiceCancelOrder);
  }, []);

  useEffect(() => {
    if (!pendingCancel || !finalTranscript) return;
    const lower = finalTranscript.toLowerCase();
    if (
      ["yes", "haan", "ha", "haanji", "yes please", "confirm", "ok", "okay"].some((word) =>
        lower.includes(word)
      )
    ) {
      handleCancelOrder();
      ...
      ["no", "nahin", "cancel", "abort", "nope"].some((word) => lower.includes(word))
    ) {
      voiceResponse && voiceResponse("Order cancellation aborted.");
      setPendingCancel(false);
      resetTranscript && resetTranscript();
    }
  }, [pendingCancel, finalTranscript]);
  ...
  const getStatusIndex = (status) => {
    const statusMap = { placed: 0, processing: 1, shipped: 2, out_for_delivery: 3, delivered: 4, cancelled: -1 };
            ...
            {selectedOrder && (
              <>
                        ...
                        {statusSteps.map((step, index) => {
                          ...
                          return (
                            <div key={step.key} className="flex flex-col items-center">
                              ...
                              <p className={`mt-2 text-xs font-medium text-center ${isCompleted ? "text-emerald-600" : "text-gray-400"}`}>
                                {step.label}
                              </p>
                            </div>
                          );
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div className="flex items-center gap-3 mb-4">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">
                          {selectedOrder.shippingAddress?.name || currentUser?.fname}
                        </p>
                      </div>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-gray-900">Rs.{((selectedOrder.paymentDetails?.amount || 0) / 100).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">Delivery Charges</span>
                      <span className="text-gray-900">Rs.{selectedOrder.deliveryCharge !== undefined ? selectedOrder.deliveryCharge.toFixed(2) : "0.00"}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">Discount</span>
                      <span className="text-green-600">-Rs.{selectedOrder.discount !== undefined ? selectedOrder.discount.toFixed(2) : "0.00"}</span>
                    </div>
                    <hr className="my-3" />
                    ...
                    <div className="mt-4 p-3 bg-green-50 rounded-lg flex items-center gap-2">
                      <span className="text-sm text-green-700">
                        {selectedOrder.paymentDetails?.payment_method_types?.[0] === "cod" ||
                        selectedOrder.paymentMethod === "cod" ||
                        selectedOrder.paymentMethod === "COD"
                          ? "Cash on Delivery"
                          : `Paid via ${
                              selectedOrder.paymentDetails?.payment_method_types?.[0]
                                ? selectedOrder.paymentDetails.payment_method_types[0].charAt(0).toUpperCase() +
                                  selectedOrder.paymentDetails.payment_method_types[0].slice(1)
                                : "Card"
                            }`}
                      </span>
                    </div>
                ...
                <div className="flex flex-wrap gap-4">
                  <button className="flex-1 min-w-[150px] px-6 py-3 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 transition-colors">
                    Need Help?
                  </button>
                  <button className="flex-1 min-w-[150px] px-6 py-3 border border-gray-300 text-gray-700 rounded-lg font-medium hover:bg-gray-50 transition-colors">
                    Download Invoice
                  </button>
                  {selectedOrder.status !== "delivered" && selectedOrder.status !== "cancelled" && (
                    <button
                      className="flex-1 min-w-[150px] px-6 py-3 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition-colors"
